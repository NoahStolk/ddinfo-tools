...
internal sealed class ObjectBindingPathsTable(FileStates fileStates, INativeFileDialog nativeFileDialog) : IPathTable
{
	...
	public void RenderPath(int index)
	{
		...
		if (ImGui.Button(Inline.Utf8($"Browse##ObjectBinding_{index}")))
			SetPath(assetInfo, path);
		ImGui.SameLine();
		if (ImGui.Button(Inline.Utf8($"Clear##ObjectBinding_{index}")) && path != null)
			path.SetPath(null);
		ImGui.SameLine();
