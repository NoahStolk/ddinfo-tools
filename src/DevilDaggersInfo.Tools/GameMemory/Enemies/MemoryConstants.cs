...
public static class MemoryConstants
{
	...
	public const int PedeSize = 6984 + 50 * 352;
	...
	public static EnemyMemory Pede { get; } = new(0x00251470, PedeSize + 50 * 352, new[] { 0x0, 0x28, 0x0 });
		...
		where TEnemy : unmanaged
	{
		return typeof(TEnemy) switch
		{
			...
			_ => throw new ArgumentException($"Unknown enemy type {typeof(TEnemy).Name}.", nameof(TEnemy)),
		};
	}
}
